...
void __ISR(_ADC_VECTOR, ipl3AUTO) _IntHandlerDrvAdc(void)
{
    ...
    for(i = 0; i < 8; i++)
    {
        SampleFilter_put(&samples, (int16_t)DRV_ADC_SamplesRead(i+offset));
    }
    
    j += 8;
    
    if (j >= SAMPLEFILTER_TAP_NUM)
    {
        j = 0;
        samplesFull = true;
        LED0Toggle();
...
// timer 1 pour cadencement APP
void __ISR(_TIMER_1_VECTOR, ipl1AUTO) IntHandlerDrvTmrInstance0(void)
{
    PLIB_INT_SourceFlagClear(INT_ID_0,INT_SOURCE_TIMER_1);
}

// timer 3 pour starter ADC. ISR non utilisée
